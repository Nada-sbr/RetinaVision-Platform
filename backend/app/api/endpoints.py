...
def register(user_in: schemas.UserCreate, db: Session = Depends(get_db)):
    ...
    user = models.User(email=user_in.email, hashed_password=hashed_pwd, full_name=user_in.full_name, role=user_in.role)
    db.add(user)
    db.commit()
    db.refresh(user)
    return user
...
def create_patient(
    patient_in: schemas.PatientCreate, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user)
):
...
def explain_image(image_id: uuid.UUID, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user)):
    ...
    try:
        ...
        pred_probs = [
            image.prediction.n_prob,
            image.prediction.d_prob,
            image.prediction.g_prob,
            image.prediction.c_prob,
            image.prediction.a_prob,
            image.prediction.h_prob,
            image.prediction.m_prob,
            image.prediction.o_prob,
        ]
...
def generate_clinical_report(
    image_id: uuid.UUID, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user)
):
    image = db.query(models.Image).filter(models.Image.id == image_id).first()
    if not image:
        raise HTTPException(status_code=404, detail="Image not found")
    ...
    today = date.today()
    age = (
        today.year - patient.birth_date.year - ((today.month, today.day) < (patient.birth_date.month, patient.birth_date.day))
    )
    ...
    # 4. Save Report in DB
    report = models.Report(image_id=image_id, report_text=report_text, status="draft")
    db.add(report)
    db.commit()
    db.refresh(report)
    return report
...
def get_clinical_history(db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user)):
    ...
    history = []
    for img in images:
        history.append({"image": img, "patient": img.patient, "prediction": img.prediction, "report": img.report})
    return history
